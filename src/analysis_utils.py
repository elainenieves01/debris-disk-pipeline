...
def read_orbital_evolution(config):
    ...
    aGP = np.full(Nsnap, np.nan)
    eGP = np.full(Nsnap, np.nan)
    iGP = np.full(Nsnap, np.nan)
    ...
    for i, sim_i in enumerate(sa):
        ...
        gp = sim_i.particles["GP"]
        aGP[i] = gp.a
        eGP[i] = gp.e
        iGP[i] = np.rad2deg(gp.inc)

        current_npl = sim_i.N_active - 2
        current_Npart = sim_i.N - sim_i.N_active

        sfdps[i] = current_npl / npl
        sfps[i] = current_Npart / Npart
